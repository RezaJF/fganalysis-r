...
test_that("get_drug_purchases handles input correctly", {
  ...
  expect_error(get_drug_purchases(phenos, "A01", finngen_ids = "FG1"), NA)
})
...
create_test_blup_data <- function(include_sex = FALSE) {
  ...
  # Add SEX column if requested
  if (include_sex) {
    # Create sex data for each individual
    sex_by_individual <- data.frame(
      FINNGENID = individuals,
      SEX = sample(c("male", "female", "UNKNOWN"), n_individuals, replace = TRUE)
    )
    lab_data <- lab_data %>% left_join(sex_by_individual, by = "FINNGENID")
  }
...
test_that("calculate_blup_slopes works correctly", {
  ...
  # Test 1: Function works with SEX column included
  drug_resp_obj_with_sex <- create_test_blup_data(include_sex = TRUE)

  blup_results <- calculate_blup_slopes(drug_resp_obj_with_sex,
                                        output_dir = temp_dir,
                                        include_sex = TRUE)

  expect_true(is.list(blup_results))

  # Check that sex was coded correctly (1=M, 2=F, 0=Other/Unknown)
  if (length(blup_results) > 0) {
    model_data <- blup_results[[1]]$model@frame
    expect_true(all(model_data$SEX_CODED %in% c(0L, 1L, 2L)))
  }

  # Test 2: Function works without SEX column when include_sex = FALSE
  drug_resp_obj_no_sex <- create_test_blup_data(include_sex = FALSE)

  blup_results_no_sex <- calculate_blup_slopes(drug_resp_obj_no_sex,
                                                output_dir = temp_dir,
                                                include_sex = FALSE)
  expect_true(is.list(blup_results_no_sex))

  # Check that SEX_CODED is all 1s in the model data
  if (length(blup_results_no_sex) > 0) {
    model_data_no_sex <- blup_results_no_sex[[1]]$model@frame
    expect_true(all(model_data_no_sex$SEX_CODED == 1L))
  }

  # Test 3: Function throws error when SEX column is missing but include_sex = TRUE
  expect_error(
    calculate_blup_slopes(drug_resp_obj_no_sex,
                          output_dir = temp_dir,
                          include_sex = TRUE),
    "SEX column not found in drug_response object"
  )

  # Test 4: Function handles minimum measurements requirement
  blup_results_strict <- calculate_blup_slopes(drug_resp_obj_with_sex,
                                                output_dir = temp_dir,
                                                min_measurements = 5,
                                                include_sex = TRUE)
  ...
  # Test 5: Function throws error for invalid input
  expect_error(
  ...
  for (concept_id in c("3004410", "3023602")) {
    file_path <- file.path(temp_dir, paste0(concept_id, "_DF13.tsv"))
